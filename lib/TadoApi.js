...
class TadoApi extends events.EventEmitter {
	...
	constructor() {
		...
		this._refreshRetry = 0; // counter to keep track of failed refresh actions
	}
		...
		return rp({
			...
			if( !isRefreshed && err.statusCode === 401 )
				return ( typeof this._username === 'string' ? this.login() : this.refreshOAuth2Token() )
					.catch(err => {
						if ((this._refreshRetry > 0) && (this._refreshRetry < 5)) {
							console.error('[TadoApi] Refresh failed, trying again in 20sec');
							// Set the retry count and try again in 20sec.
							this._refreshRetry += 1;
							setTimeout(this._call( method, path, data, isRefreshed), 20*1000);
						} else {
